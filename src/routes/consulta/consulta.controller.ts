...
export class ConsultaController {
    ...
    async createconsulta(@Body() dto: consultaDto){
        ...
        try{
            ...
            if(e.sqlMessage == "Cannot add or update a child row: a foreign key constraint fails (`med_cover`.`consulta`, CONSTRAINT `FK_0ccaf3cf7888266ee10a8b60c65` FOREIGN KEY (`consultorioId`) REFERENCES `consultorio` (`id`))")
            {   
                throw new HttpException('Consultorio não encontrado', HttpStatus.NOT_FOUND);
            }

            if(e.sqlMessage == "Cannot add or update a child row: a foreign key constraint fails (`med_cover`.`consulta`, CONSTRAINT `FK_f3f4259b920b9dc53adad1a30d1` FOREIGN KEY (`medicoId`) REFERENCES `medico` (`id`))")
            {
                throw new HttpException('Medico não encontrado', HttpStatus.NOT_FOUND);
            }

            if(e.sqlMessage == "Cannot add or update a child row: a foreign key constraint fails (`med_cover`.`consulta`, CONSTRAINT `FK_1fe809eb27c3a3ddfc8be164fe7` FOREIGN KEY (`usuarioId`) REFERENCES `usuario` (`id`))")
            {
    ...
    @Delete(':id')
    async deleteconsulta(@Param('id') params: number){//idDto){
        console.log(params)
